...
void Camera::DrawTexture(SGD::Point position, float rotation, SGD::HTexture m_hImage, bool flipped)
{
	...
	{ position.x - m_ptCameraPosition.x, position.y - m_ptCameraPosition.y }, rotation, {}, {}, { m_fScale, m_fScale });
}
...
{
	m_ptCameraPosition.x = (m_pPlayer->GetPosition().x - Game::GetInstance()->GetScreenWidth()  / 2)  /*/ m_fScale*/; // Divide by Scale
